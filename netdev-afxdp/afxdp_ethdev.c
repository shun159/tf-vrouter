...
xsk_configure_umem(struct vr_afxdp_xsk_socket_info *xsk)
{
    ...
    memcpy(&xsk->bpool_params, &bpool_params_default, sizeof(struct bpool_params));
    memcpy(&xsk->bpool->umem_cfg, &umem_cfg_default, sizeof(struct xsk_umem_config));
...
xsk_configure_queue(struct vr_afxdp_ethdev *ethdev, __u32 queue_id)
{
    ...
    if (if_indextoname(ethdev->os_ifidx, devname) == NULL) {
        fprintf(stderr, "ifindex %d to devname failed (%s)\n", ethdev->os_ifidx, strerror(errno));
        goto err_free_umem;
    }
...
xsk_configure(struct vr_afxdp_ethdev *ethdev)
{
    ...
    for (i = 0; i < n_rxq; i++) {
        ...
        if (ret) {
            fprintf(stderr, "xsk_configure_queue failed: ifidx %d: queue_id:%d\n", ethdev->os_ifidx, i);
            return -1;
        }
    }
...
static inline void
prepare_tx_queue(struct vr_afxdp_xsk_socket_info *xi, struct vr_afxdp_tx_cache *c)
{
