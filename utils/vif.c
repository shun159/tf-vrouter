...
vr_interface_print_header(void)
{
  ...
  for (i = 0; i < array_size; i++) {
    ...
    printf("%s=%s",
           flag_metadata[i].vuf_flag_symbol,
...
static void
vr_interface_pe_counters_print(const char *title,
                               bool print_always,
                               uint64_t packets,
                               uint64_t errors)
{
...
static void
vr_interface_e_per_lcore_counters_print(const char *title,
                                        bool print_always,
                                        uint64_t *errors,
                                        uint32_t size)
{
...
{
  char ip6_addr[INET6_ADDRSTRLEN], ip_addr[INET_ADDRSTRLEN], name[50] = {0},
                                                             ip6_ip[16];
  ...
  if (req->vifr_flags & VIF_FLAG_PMD) {
    ...
    case VIF_TYPE_MONITORING:
      printf("Monitoring: %s for vif%d/%d",
             req->vifr_name,
             req->vifr_rid,
             req->vifr_os_idx);
      break;
  ...
  if ((req->vifr_type == VIF_TYPE_PHYSICAL) &&
    ...
    if (req->vifr_src_mac_size && req->vifr_src_mac) {
      for (i = 0; i < (req->vifr_src_mac_size / VR_ETHER_ALEN); i = i + 1) {
        printf(", " MAC_FORMAT,
               MAC_VALUE((uint8_t *)(req->vifr_src_mac + (i * VR_ETHER_ALEN))));
  ...
  if (platform == DPDK_PLATFORM || platform == VTEST_PLATFORM) {
    ...
    vr_interface_pe_counters_print("RX queue ",
                                   print_zero,
                                   req->vifr_queue_ipackets,
                                   req->vifr_queue_ierrors);

    vr_interface_e_per_lcore_counters_print(
        "RX queue",
        print_zero,
  ...
  vr_interface_pbem_counters_print("RX",
                                   true,
                                   req->vifr_ipackets,
                                   req->vifr_ibytes,
                                   req->vifr_ierrors,
                                   0);
  vr_interface_pbem_counters_print("TX",
                                   true,
                                   req->vifr_opackets,
                                   req->vifr_obytes,
                                   req->vifr_oerrors,
  ...
  if (platform == DPDK_PLATFORM || platform == VTEST_PLATFORM) {
    vr_interface_pe_counters_print("TX queue ",
                                   print_zero,
  ...
  if (req->vifr_fat_flow_protocol_port_size) {
    ...
    for (i = 0; i < req->vifr_fat_flow_protocol_port_size; i++) {
      ...
      aggr_data =
          VIF_FAT_FLOW_PREFIX_AGGR_DATA(req->vifr_fat_flow_protocol_port[i]);
      if (!proto) {
        proto = port;
        port = 0;
      }
...
list_rate_print(vr_interface_req *req)
{
  ...
  rx_errors = (req->vifr_dev_ierrors + req->vifr_port_ierrors +
               req->vifr_queue_ierrors + req->vifr_ierrors);
  tx_errors = (req->vifr_dev_oerrors + req->vifr_port_oerrors +
               req->vifr_queue_oerrors + req->vifr_oerrors);

  printed =
      printf("%s: %s", vr_get_if_type_string(req->vifr_type), req->vifr_name);
  for (; printed < 30; printed++)
    printf(" ");
  printed = printf("vif%d/%d", req->vifr_rid, req->vifr_idx);
...
rate_stats_diff(vr_interface_req *req, vr_interface_req *prev_req)
{
  ...
  CORRECT_ERROR_CNT(&(req->vifr_queue_ierrors),
                    &(prev_req->vifr_queue_ierrors));
  ...
  CORRECT_ERROR_CNT(&(req->vifr_queue_oerrors),
                    &(prev_req->vifr_queue_oerrors));
...
rate_stats(struct nl_client *cl, unsigned int vr_op)
{
  ...
  while (true) {
    while (!is_stdin_hit() || get_set) {
      ...
      print_number_interface =
          (print_number_interface > MAX_OUTPUT_IF ? MAX_OUTPUT_IF
                                                  : print_number_interface);
