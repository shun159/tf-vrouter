...
static int kindex_set, type_set, transport_set, help_set, set_set, vlan_set, dhcp_set, mac_learn_set, uuc_flood_set;
static int vrf_set, mac_set, delete_set, policy_set, pmd_set, vindex_set, pci_set, nh_id, nh_set;
...
static void
vr_interface_pbem_counters_print(const char *title,
                                 bool print_always,
                                 uint64_t packets,
                                 uint64_t bytes,
                                 uint64_t errors,
                                 uint64_t nombufs)
{
    if (print_always || packets || bytes || errors) {
        ...
        printf("%s packets:%" PRId64 "  bytes:%" PRId64 " errors:%" PRId64, title, packets, bytes, errors);
        vr_interface_nombufs_print(nombufs);
    }
}

static void
vr_interface_pesm_counters_print(const char *title,
                                 bool print_always,
                                 uint64_t packets,
                                 uint64_t errors,
                                 uint64_t syscalls,
...
static void
vr_interface_pe_counters_print(const char *title, bool print_always, uint64_t packets, uint64_t errors)
{
    if (print_always || packets || errors) {
        vr_interface_print_head_space();
        vr_interface_core_print();
...
static void
vr_interface_e_per_lcore_counters_print(const char *title, bool print_always, uint64_t *errors, uint32_t size)
{
    unsigned int i;
...
vr_interface_fabric_info(vr_interface_req *req)
{
    ...
    printf("Fabric Interface: %s  Status: %s  Driver: %s\n",
           req->vifr_fab_name,
           fabric_link[(req->vifr_intf_status & 0x01)],
    ...
    for (i = 0; i < req->vifr_num_bond_slave; i++) {
        ...
        printf("Slave Interface(%d): %s  Status: %s  Driver: %s\n",
               i,
               p_name,
               fabric_link[(req->vifr_intf_status >> (i + 1)) & 0x01],
               p_drv_name);
...
list_get_print(vr_interface_req *req)
{
    ...
    vr_interface_print_head_space();
    printf("Vrf:%d Mcast Vrf:%d Flags:%s QOS:%d Ref:%d",
           req->vifr_vrf,
           req->vifr_mcast_vrf,
           req->vifr_flags ? vr_if_flags(req->vifr_flags) : "NULL",
           req->vifr_qos_map_index,
           req->vifr_ref_cnt);
    ...
    if (platform == DPDK_PLATFORM || platform == VTEST_PLATFORM) {
        vr_interface_pbem_counters_print("RX device",
                                         print_zero,
                                         req->vifr_dev_ipackets,
                                         req->vifr_dev_ibytes,
                                         req->vifr_dev_ierrors,
                                         req->vifr_dev_inombufs);
        vr_interface_pesm_counters_print("RX port  ",
                                         print_zero,
                                         req->vifr_port_ipackets,
                                         req->vifr_port_ierrors,
                                         req->vifr_port_isyscalls,
                                         req->vifr_port_inombufs);
        vr_interface_pe_counters_print("RX queue ", print_zero, req->vifr_queue_ipackets, req->vifr_queue_ierrors);

        vr_interface_e_per_lcore_counters_print("RX queue",
                                                print_zero,
    ...
    vr_interface_pbem_counters_print("RX", true, req->vifr_ipackets, req->vifr_ibytes, req->vifr_ierrors, 0);
    vr_interface_pbem_counters_print("TX", true, req->vifr_opackets, req->vifr_obytes, req->vifr_oerrors, 0);
    ...
    if (platform == DPDK_PLATFORM || platform == VTEST_PLATFORM) {
        vr_interface_pe_counters_print("TX queue ", print_zero, req->vifr_queue_opackets, req->vifr_queue_oerrors);
        vr_interface_pesm_counters_print("TX port  ",
                                         print_zero,
                                         req->vifr_port_opackets,
                                         req->vifr_port_oerrors,
                                         req->vifr_port_osyscalls,
                                         0);
        vr_interface_pbem_counters_print("TX device",
                                         print_zero,
...
list_rate_print(vr_interface_req *req)
{
    ...
    rx_errors = (req->vifr_dev_ierrors + req->vifr_port_ierrors + req->vifr_queue_ierrors + req->vifr_ierrors);
    tx_errors = (req->vifr_dev_oerrors + req->vifr_port_oerrors + req->vifr_queue_oerrors + req->vifr_oerrors);
...
rate_process(vr_interface_req *req, vr_interface_req *prev_req)
{
    ...
    if (first_rate_iter) {
        ...
        *prev_req = *req;
        ...
        if (!prev_req->vifr_queue_ierrors_to_lcore) {
            prev_req->vifr_queue_ierrors_to_lcore = malloc(req->vifr_queue_ierrors_to_lcore_size * sizeof(uint64_t));
            if (!prev_req->vifr_queue_ierrors_to_lcore)
                return;
        }
    ...
    rate_req_temp = *req;
    rate_req_temp.vifr_queue_ierrors_to_lcore = calloc(req->vifr_queue_ierrors_to_lcore_size, sizeof(uint64_t));
 ...
 * In case, when we have sent SANDESH_OP_DUMP (usually --list parameter) msg to
 * nl_client, then sandesh_decode calls vr_interface_req_process in "loop".
...
interface_req_process(void *s)
{
    ...
    if (req->h_op == SANDESH_OP_RESET) {
        if (req->vifr_idx == -1) {
            if (req->vifr_core != 0) {
                printf("\nVif stats cleared successfully on core %d for all "
                       "interfacess \n\n",
                       req->vifr_core - 1);
            } else {
                printf("\nVif stats cleared successfully on all cores for all "
                       "interfaces \n\n");
            }
        } else {
            if (req->vifr_core != 0) {
...
parse_long_opts(int option_index, char *opt_arg)
{
    ...
    case NH_OPT_INDEX:
        if (is_number(opt_arg))
            nh_id = safer_strtoul(opt_arg, NULL, 0);
        else
            usage_internal("nh set --nexthop");
...
validate_options(void)
{
    ...
    if (get_set) {
        if ((sum_opt > 1) && (sum_opt != 3) && (sum_opt != 4) && (!kindex_set && !core_set && !rate_set && !drop_set))
            Usage();
        return;
    }
    if (delete_set) {
...
rate_stats(struct nl_client *cl, unsigned int vr_op)
{
    ...
    while (true) {
        while (!is_stdin_hit() || get_set) {
            ...
            print_number_interface = (print_number_interface > MAX_OUTPUT_IF ? MAX_OUTPUT_IF : print_number_interface);
            ...
            if (list_set) {
                printf("Key 'q' for quit, key 'k' for previous page, key 'j' for next "
                       "page.\n");
            }
        ...
        case 'k':
            local_ignore_number_interface = (local_ignore_number_interface - local_print_number_interface <= 0)
                                                ? 0
                                                : (local_ignore_number_interface - local_print_number_interface);
            break;

        case 'j':
            local_ignore_number_interface = (local_ignore_number_interface + local_print_number_interface);
            break;
...
main(int argc, char *argv[])
{
    ...
    while ((opt = getopt_long(argc, argv, "ba:c:d:g:klm:t:T:v:p:C:DPi:s:", long_options, &option_index)) >= 0) {
        switch (opt) {
        case 'a':
            add_set = 1;
        ...
        case 'n':
            nh_set = 1;
            parse_long_opts(NH_OPT_INDEX, optarg);
            break;
    ...
    if (!rate_set) {
         ...
         * We dont have to write enter (newline) for getting character from
         * terminal.
         *
         */
