  ...
  if (req->nhr_type == NH_RCV) {
    ...
    if (!(req->nhr_flags & NH_FLAG_TUNNEL_PBB)) {
      if (req->nhr_family == AF_INET) {
        ...
        printf("Sip: %s",
               inet_ntop(AF_INET6,
                         (struct in6_addr *)req->nhr_tun_sip6,
                         in6_dst,
                         sizeof(in6_dst)));
        printf(" Dip: %s",
               inet_ntop(AF_INET6,
                         (struct in6_addr *)req->nhr_tun_dip6,
                         in6_dst,
                         sizeof(in6_dst)));
      }
    }
    ...
    if (req->nhr_flags & NH_FLAG_TUNNEL_PBB) {
      ...
      printf("Bmac:" MAC_FORMAT " Label:%d",
             MAC_VALUE((uint8_t *)req->nhr_pbb_mac),
             i);
    }
...
op_retry:
  ...
  case SANDESH_OP_ADD:
    if (flags & NH_FLAG_TUNNEL_PBB) {
      ...
      ret = vr_send_nexthop_composite_add(cl,
                                          0,
                                          nh_id,
                                          flags,
                                          vrf_id,
      ...
      ret =
          vr_send_nexthop_add(cl, 0, type, nh_id, flags, vrf_id, if_id, family);
    }
...
{
  printf(
      "Usage: [--create <nhid> create nexthop\n"
...
static struct option long_options[] = {
    ...
    [L3_VXLAN_OPT_IND] = {"l3_vxlan",
                          required_argument,
                          &opt[L3_VXLAN_OPT_IND],
                          1},
    ...
    [SOCK_DIR_OPT_IND] = {"sock-dir",
                          required_argument,
                          &opt[SOCK_DIR_OPT_IND],
                          1},
