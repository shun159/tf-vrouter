...
static int nh_set, command, type, dump_marker = -1, flow_lookup_set;
...
static int vr_nh_op(struct nl_client *cl,
                    int command,
                    int type,
                    uint32_t nh_id,
                    uint32_t *if_id,
...
nexthop_req_process(void *s_req)
{
    ...
    if (req->nhr_type == NH_RCV) {
        ...
        nh_print_newline_header();
        printf("EncapFmly:%04x Oif:%d Len:%d", req->nhr_encap_family, req->nhr_encap_oif_id[0], req->nhr_encap_size);
        nh_print_newline_header();
        printf("Encap Data: ");
        ...
        if (!(req->nhr_flags & NH_FLAG_TUNNEL_PBB)) {
            if (req->nhr_flags & NH_FLAG_TUNNEL_UNDERLAY_ECMP) {
                for (i = 0; i < req->nhr_encap_oif_id_size; i++) {
                    printf("Oif:%d EncapValid:%d ", req->nhr_encap_oif_id[i], req->nhr_encap_valid[i]);
                    if (req->nhr_encap_valid[i]) {
                        printf("Len:%d Data:", req->nhr_encap_len);
                        for (j = 0; j < req->nhr_encap_len; j++) {
                            printf("%02x ", (unsigned char)req->nhr_encap[i * req->nhr_encap_len + j]);
                        }
                    } else
                ...
                printf("Oif:%d Len:%d Data:", req->nhr_encap_oif_id[0], req->nhr_encap_size);
                for (i = 0; i < req->nhr_encap_size; i++) {
                    printf("%02x ", (unsigned char)req->nhr_encap[i]);
                }
                nh_print_newline_header();
        ...
        if (!(req->nhr_flags & NH_FLAG_TUNNEL_PBB)) {
            if (req->nhr_family == AF_INET) {
                ...
                printf("Sip: %s", inet_ntop(AF_INET6, (struct in6_addr *)req->nhr_tun_sip6, in6_dst, sizeof(in6_dst)));
                printf(" Dip: %s", inet_ntop(AF_INET6, (struct in6_addr *)req->nhr_tun_dip6, in6_dst, sizeof(in6_dst)));
            }
        }
        ...
        if (req->nhr_type == NH_COMPOSITE) {
            for (i = 0; i < req->nhr_nh_list_size; i++) {
                ...
                if (req->nhr_nh_list[i] == -1)
                    continue;
                vr_nh_op(cl, command, type, req->nhr_nh_list[i], if_id, vrf_id, dst_mac, src_mac, sip, dip, flags);
            }
        }

        if ((req->nhr_flags & NH_FLAG_INDIRECT) && (req->nhr_nh_list_size)) {
            vr_nh_op(cl, command, type, req->nhr_nh_list[0], if_id, vrf_id, dst_mac, src_mac, sip, dip, flags);
        }
    }
}
...
op_retry:
    ...
    case SANDESH_OP_ADD:
        if (flags & NH_FLAG_TUNNEL_PBB) {
            ret = vr_send_pbb_tunnel_add(cl, 0, nh_id, flags, vrf_id, dst[0], comp_nh[0], lbl[0]);
        } else if ((type == NH_ENCAP) || (type == NH_TUNNEL)) {
            ret = vr_send_nexthop_encap_tunnel_add(cl,
                                                   0,
                                                   type,
                                                   nh_id,
                                                   flags,
            ...
            ret = vr_send_nexthop_composite_add(cl, 0, nh_id, flags, vrf_id, comp_nh_ind, comp_nh, lbl, family);
        } else {
...
cmd_usage()
{
    printf("Usage: [--create <nhid> create nexthop\n"
           ...
           "                       3 - tunnel, 4 - resolve, 5 - discard, 6 - "
           "Composite\n"
           ...
           "                        [--ind indirect flag]\n"
           "                    [--oif <if_id1,if_id2,...> comma seperated list of "
           "out going interface indices(max 3 in case of L3 multihoming)]\n"
           "                    [--smac <xx:xx:xx:xx:xx:xx,xx:xx:xx:xx:xx:xx,...> "
           "comma seperated list of source mac(max 3 in case of L3 multihoming)]\n"
           "                    [--dmac <xx:xx:xx:xx:xx:xx,xx:xx:xx:xx:xx:xx,...> "
           "comma seperated list of destination mac(max 3 in case of L3 "
           "multihoming)]\n"
           "                    [--sip <x.x.x.x> source ip of tunnel] \n"
           "                    [--dip <x.x.x.x> destination ip of tunnel ]\n"
           "                    [--udp Udptunnel ]\n"
           "                        [--sport <port> source port of udp tunnel]\n"
           "                        [--dport <port> destination port of udp "
           "tunnel]\n"
           "                    [--vxlan Vxlan Tunnel]\n"
           "                        [--sport <port> source port of vxlan tunnel]\n"
           "                        [--dport <port> destination port of vxlan "
           "tunnel]\n"
           "                        [--l3_vxlan <xx:xx:xx:xx:xx:xx> Remote EVPN-5 "
           "vRouter mac]\n"
           "                [RESOLVE_NH options]\n"
           "                [DISCARD_NH options]\n"
...
static struct option long_options[] = {
    ...
    [FLOW_LOOKUP_OPT_IND] = {"flow_lookup", no_argument, &opt[FLOW_LOOKUP_OPT_IND], 1},
...
validate_options(void)
{
    ...
    case SANDESH_OP_ADD:
        ...
        if (opt_set(FLOW_LOOKUP_OPT_IND))
            flags |= NH_FLAG_FLOW_LOOKUP;

        printf("nh_type: %d NH_L2_RCV: %d\n", type, NH_L2_RCV);
        if (type == NH_RCV) {
            ...
            if (!(flags & (NH_FLAG_TUNNEL_UDP_MPLS | NH_FLAG_TUNNEL_UDP | NH_FLAG_TUNNEL_VXLAN | NH_FLAG_TUNNEL_PBB)))
