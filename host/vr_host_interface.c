...
static unsigned int hif_udp_tx(struct vr_hinterface *hif,
                               struct vr_hpacket *hpkt) {
  int i = 0;
  struct msghdr msg;
...
int vr_hif_udp_create(struct vr_hinterface *hif, unsigned int vif_type) {
  ...
  sock_addr.sin_family = AF_INET;
  sock_addr.sin_port =
      htons(port - HIF_SOURCE_UDP_PORT_START + HIF_DESTINATION_UDP_PORT_START);
...
struct vr_hinterface *vr_hinterface_create(unsigned int index,
...
static int vr_lib_interface_rx(struct vr_interface *vif,
                               struct vr_packet *pkt) {
  return 0;
}

static int vr_lib_interface_tx(struct vr_interface *vif,
                               struct vr_packet *pkt) {
...
static int vr_lib_interface_add_tap(struct vr_interface *vif,
