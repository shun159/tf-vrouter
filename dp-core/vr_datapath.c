...
extern struct vr_nexthop *vr_inet_src_lookup(unsigned short vrf,
                                             struct vr_packet *pkt);

mac_response_t
vr_get_proxy_mac(struct vr_packet *pkt,
...
static void
vr_arp_proxy(struct vr_arp *sarp,
             struct vr_packet *pkt,
             struct vr_forwarding_md *fmd,
             unsigned char *dmac)
{
...
static int
vr_handle_arp_request(struct vr_arp *sarp,
                      struct vr_packet *pkt,
                      struct vr_forwarding_md *fmd,
 ...
 */
static int
vr_handle_arp_reply(struct vr_arp *sarp,
                    struct vr_packet *pkt,
                    struct vr_forwarding_md *fmd)
{
  struct vr_interface *vif = pkt->vp_if;
 ...
 */
int
vif_plug_mac_request(struct vr_interface *vif,
                     struct vr_packet *pkt,
                     struct vr_forwarding_md *fmd)
{
...
int
vr_arp_input(struct vr_packet *pkt,
             struct vr_forwarding_md *fmd,
             unsigned char *eth_dmac)
{
  int handled = 1;
 ...
 */
unsigned int
vr_virtual_input(unsigned short vrf,
...
unsigned int
vr_fabric_input(struct vr_interface *vif,
                struct vr_packet *pkt,
                struct vr_forwarding_md *fmd,
                unsigned short vlan_id)
{
...
vr_pbb_decode(struct vr_packet *pkt, struct vr_forwarding_md *fmd)
{
  ...
  pbb_size =
      __vr_pbb_decode((struct vr_eth *)pkt_data(pkt), pkt_head_len(pkt), fmd);
  if (pbb_size <= 0) {
