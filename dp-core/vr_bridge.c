                    ...
                    struct vr_nexthop *ingress_nh)
{
  ...
  if (vif_is_virtual(ingress_vif)) {
    ...
    return (ingress_nh && (ingress_nh->nh_flags & NH_FLAG_UNKNOWN_UC_FLOOD));
  }
...
bridge_add(unsigned int router_id, unsigned int vrf, uint8_t *mac, int nh_id)
{
  struct vr_bridge_entry *be;
  struct vr_bridge_entry_key key;
...
__bridge_table_add(struct vr_route_req *rt)
{
  ...
  be = bridge_add(rt->rtr_req.rtr_rid,
                  rt->rtr_req.rtr_vrf_id,
                  rt->rtr_req.rtr_mac,
  ...
  for (i = 0; i < 10; i++) {
    flags = be->be_flags;
    if (vr_sync_bool_compare_and_swap_16u(&be->be_flags,
                                          flags,
                                          VR_BE_VALID_FLAG |
                                              rt->rtr_req.rtr_label_flags)) {
      ret = 0;
...
static void
bridge_table_entry_free(vr_htable_t table,
                        vr_hentry_t *hentry,
                        unsigned int index,
                        void *data)
{
...
__bridge_lookup(unsigned int vrf_id, struct vr_route_req *rt)
{
  ...
  if (rt->rtr_req.rtr_index != VR_BE_INVALID_INDEX) {
    be = (struct vr_bridge_entry *)vr_htable_get_hentry_by_index(
        vn_rtable,
...
vr_bridge_set_route_flags(struct vr_bridge_entry *be, unsigned short flags)
{
  ...
  if (be) {
    ...
    if (be_flags & VR_BE_VALID_FLAG) {
      ...
      be_flags = vr_sync_val_compare_and_swap_16u(&be->be_flags,
                                                  be_flags,
                                                  flags | be_flags);
...
vr_hentry_key
bridge_entry_key(vr_htable_t table, vr_hentry_t *entry, unsigned int *key_len)
{
  struct vr_bridge_entry *be =
      CONTAINER_OF(be_hentry, struct vr_bridge_entry, entry);
...
{
  vr_bridge_table_data *data =
      vr_zalloc(sizeof(vr_bridge_table_data), VR_BRIDGE_TABLE_DATA_OBJECT);
  if (!data)
    return NULL;

  if (vr_bridge_table_path) {
    data->btable_file_path =
        vr_zalloc(VR_UNIX_PATH_MAX, VR_BRIDGE_TABLE_DATA_OBJECT);
    if (!data->btable_file_path) {
      goto exit_func;
    }
  }
...
bridge_table_init(struct vr_rtable *rtable, struct rtable_fspec *fs)
{
  ...
  if (!vr_bridge_table && vr_huge_page_mem_get) {
    vr_bridge_table = vr_huge_page_mem_get(
        (VR_BRIDGE_TABLE_SIZE + VR_BRIDGE_OFLOW_TABLE_SIZE),
        &vr_bridge_table_path);
    if (vr_bridge_table)
      vr_bridge_otable =
          (unsigned char *)vr_bridge_table + VR_BRIDGE_TABLE_SIZE;
  }

  rtable->algo_data = vr_htable_attach(vrouter_get(0),
                                       vr_bridge_entries,
                                       vr_bridge_table,
                                       vr_bridge_oentries,
                                       vr_bridge_otable,
                                       sizeof(struct vr_bridge_entry),
                                       sizeof(struct vr_bridge_entry_key),
                                       0,
                                       bridge_entry_key);
...
void
bridge_table_deinit(struct vr_rtable *rtable,
...
mac_learn_t
vr_bridge_learn(struct vrouter *router,
                ...
                struct vr_forwarding_md *fmd)
{
  ...
  if (trap) {
    ...
    vr_trap(pkt_c,
            fmd->fmd_dvrf,
            trap_reason,
            (void *)&be->be_hentry.hentry_index);
  }
...
unsigned int
vr_bridge_input(struct vrouter *router,
                struct vr_packet *pkt,
                struct vr_forwarding_md *fmd)
{
  int reason, handled;
  ...
  if (!fmd->fmd_to_me) {
     ...
     */
    if (nh && nh->nh_flags & NH_FLAG_TUNNEL_PBB) {
...
void
vr_compute_size_bridge_otable(void)
{
