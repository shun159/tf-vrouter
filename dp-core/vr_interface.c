...
static mac_response_t vm_mac_request(struct vr_interface *,
                                     struct vr_packet *,
...
extern struct vr_interface *
vif_bridge_get_sub_interface(vr_htable_t, unsigned short, unsigned char *);
extern int
vif_bridge_get_index(struct vr_interface *, struct vr_interface *, uint8_t *);
extern int vif_bridge_init(struct vr_interface *);
extern void vif_bridge_deinit(struct vr_interface *);
extern int
vif_bridge_delete(struct vr_interface *, struct vr_interface *, uint8_t *);
extern int
vif_bridge_add(struct vr_interface *, struct vr_interface *, uint8_t *);
extern void vhost_remove_xconnect(void);
extern void vr_drop_stats_get_vif_stats(vr_drop_stats_req *,
                                        struct vr_interface *);
...
static inline void
fat_flow_ipv6_plen_to_mask(uint16_t plen,
                           uint64_t *high_mask,
                           uint64_t *low_mask)
{
  if (plen > 64) {
...
static int
vif_discard_tx(struct vr_interface *vif,
...
static int
vif_discard_rx(struct vr_interface *vif,
               struct vr_packet *pkt,
 ...
 */
static int
vif_cmn_rewrite(struct vr_interface *vif,
                struct vr_packet **pkt,
                struct vr_forwarding_md *fmd,
...
vr_interface_service_enable(struct vr_interface *vif)
{
  ...
  if (!vif->vif_vrf_table) {
    vif->vif_vrf_table =
        vr_malloc(sizeof(struct vr_vrf_assign) * VIF_VRF_TABLE_ENTRIES,
...
static void
vr_pkt_dump(struct vr_packet *pkt)
{
  pkt_dump(pkt);
...
vr_get_arp_xconnect_interface_mh(struct vr_interface *vif,
                                 struct vr_packet *pkt)
{
...
static int
vr_deliver_os_interface(struct vr_interface *vif,
                        struct vr_packet *pkt,
                        struct vr_forwarding_md *fmd)
{
...
int
vif_xconnect(struct vr_interface *vif,
             struct vr_packet *pkt,
             struct vr_forwarding_md *fmd)
{
...
static inline void
vif_mirror(struct vr_interface *vif,
           struct vr_packet *pkt,
           struct vr_forwarding_md *fmd,
           unsigned int txrx_mirror)
{
...
/* agent driver */
static int
agent_set_rewrite(struct vr_interface *vif,
                  struct vr_packet **pkt,
                  struct vr_forwarding_md *fmd,
                  unsigned char *rewrite,
                  unsigned short len)
...
static int
agent_rx(struct vr_interface *vif,
         struct vr_packet *pkt,
         unsigned short vlan_id __attribute__unused__)
{
  unsigned short cmd;
  ...
  case AGENT_CMD_ROUTE:
     ...
     */
    agent_vif =
        __vrouter_get_interface(vrouter_get(0), ntohs(hdr->hdr_ifindex));
    /*
    ...
    vr_virtual_input(ntohs(hdr->hdr_vrf),
                     agent_vif,
                     pkt,
                     &fmd,
                     VLAN_ID_INVALID);
...
static int
agent_tx(struct vr_interface *vif,
         struct vr_packet *pkt,
         struct vr_forwarding_md *fmd)
{
  int ret;
...
static int
agent_send(struct vr_interface *vif, struct vr_packet *pkt, void *ifspecific)
{
  ...
  case AGENT_TRAP_FLOW_ACTION_HOLD:
    if (params->trap_param) {
      ...
      hdr->hdr_cmd_param_4 =
          htonl((fta->vfta_stats.flow_bytes_oflow |
...
static int
agent_drv_add(struct vr_interface *vif, vr_interface_req *vifr)
{
  int ret;
...
vhost_mac_request(struct vr_interface *vif,
                  struct vr_packet *pkt,
                  struct vr_forwarding_md *fmd,
                  unsigned char *dmac)
{
...
static int
vhost_rx(struct vr_interface *vif,
         struct vr_packet *pkt,
         unsigned short vlan_id __attribute__unused__)
{
  struct vr_forwarding_md fmd;
...
static int
vhost_tx(struct vr_interface *vif,
         struct vr_packet *pkt,
         struct vr_forwarding_md *fmd)
{
  int ret;
...
static int
vhost_drv_add(struct vr_interface *vif, vr_interface_req *vifr)
{
  int ret = 0, i;
...
/* vlan driver */
static int
vlan_rx(struct vr_interface *vif,
        struct vr_packet *pkt,
        unsigned short vlan_id __attribute__unused__)
{
  int8_t tos;
...
static int
vlan_tx(struct vr_interface *vif,
        struct vr_packet *pkt,
        struct vr_forwarding_md *fmd)
{
...
vlan_drv_add(struct vr_interface *vif, vr_interface_req *vifr)
{
  ...
  if (vifr->vifr_src_mac_size && vifr->vifr_src_mac) {
    ...
    if (!vif->vif_src_mac) {
      vif->vif_src_mac =
          vr_zalloc((VIF_SRC_MACS * VR_ETHER_ALEN), VR_INTERFACE_MAC_OBJECT);
      if (!vif->vif_src_mac)
        return -ENOMEM;
    }
    ...
    if (!pvif->vif_sub_interfaces) {
      pvif->vif_sub_interfaces =
          vr_zalloc(VLAN_ID_MAX * sizeof(struct vr_interface *),
                    VR_INTERFACE_OBJECT);
      if (!pvif->vif_sub_interfaces)
        return -ENOMEM;
    }
  ...
  if (!vif->vif_src_mac) {
    ...
    return vlan_sub_interface_manage(pvif,
                                     vif,
                                     (vifr->vifr_src_mac_size / VR_ETHER_ALEN),
                                     vifr->vifr_src_mac);
  }
...
/* eth driver */
static int
vm_srx(struct vr_interface *vif, struct vr_packet *pkt, unsigned short vlan_id)
{
  unsigned short vrf;
  struct vr_interface_stats *stats = vif_get_stats(vif, pkt->vp_cpu);
  struct vr_forwarding_md fmd;
...
static mac_response_t
vm_mac_request(struct vr_interface *vif,
               struct vr_packet *pkt,
               struct vr_forwarding_md *fmd,
               unsigned char *dmac)
{
...
static int
vm_rx(struct vr_interface *vif, struct vr_packet *pkt, unsigned short vlan_id)
{
  struct vr_forwarding_md fmd;
  ...
  if (vlan_id != VLAN_ID_INVALID && vlan_id < VLAN_ID_MAX) {
    if (vif->vif_btable) {
      sub_vif =
          vif_bridge_get_sub_interface(vif->vif_btable, vlan_id, eth->eth_smac);
    } else {
...
static int
tun_rx(struct vr_interface *vif, struct vr_packet *pkt, unsigned short vlan_id)
{
 ...
 */
static int
ipsec_rx(struct vr_interface *vif,
         struct vr_packet *pkt,
         unsigned short vlan_id)
{
  ...
  for (i = 0; i < VR_MAX_PHY_INF; i++) {
    if (router->vr_eth_if[i] &&
        (memcmp(vif->vif_mac, router->vr_eth_if[i]->vif_mac, VR_ETHER_ALEN) ==
         0)) {
...
static int
eth_set_rewrite(struct vr_interface *vif,
                struct vr_packet **pkt,
                struct vr_forwarding_md *fmd,
                unsigned char *rewrite,
                unsigned short len)
{
  ...
  if (((*pkt)->vp_if->vif_type == VIF_TYPE_HOST) &&
      ...
      (memcmp((*pkt)->vp_if->vif_mac,
              (pkt_data(*pkt) - VR_ETHER_HLEN),
              VR_ETHER_ALEN) != 0)) {
    vr_preset(*pkt);
    return 0;
  }
...
static mac_response_t
eth_mac_request(struct vr_interface *vif,
                struct vr_packet *pkt,
                struct vr_forwarding_md *fmd,
                unsigned char *dmac)
{
...
static int
eth_tx(struct vr_interface *vif,
       struct vr_packet *pkt,
       struct vr_forwarding_md *fmd)
{
...
static int
eth_drv_add(struct vr_interface *vif, vr_interface_req *vifr)
{
  int ret = 0;
   ...
   */
  if ((!(vif->vif_flags & VIF_FLAG_VHOST_PHYS)) || (vif->vif_bridge[0]) ||
      (hif_ops->hif_get_encap(vif) == VIF_ENCAP_TYPE_L3_DECRYPT)) {
    ret = hif_ops->hif_add_tap(vif, vifr);
    if (ret)
      hif_ops->hif_del(vif);
...
struct vr_interface *
vrouter_get_vhost_interface(struct vrouter *router)
{
  struct vr_interface *vif;
    ...
    for (i = 0; i < router->vr_num_phy_interfaces; i++) {
      ...
      if ((!vifr->vifr_cross_connect_idx) ||
        ...
        vr_printf("Vrouter: %s:%d vifr_idx: %d vifr_cross_connect_idx=%d\n",
                  ...
                  __LINE__,
                  vifr->vifr_idx,
                  vifr->vifr_cross_connect_idx ? vifr->vifr_cross_connect_idx[i]
                                               : 0);
        return -EINVAL;
      }
      eth_vif[i] =
          __vrouter_get_interface_os(router, vifr->vifr_cross_connect_idx[i]);
#endif
      if (!eth_vif[i]) {
...
vif_set_flags(struct vr_interface *vif, vr_interface_req *req)
{
  ...
  if ((req->vifr_flags & VIF_FLAG_MAC_LEARN) || (vif_is_fabric(vif))) {
    if (!vif->vif_bridge_table_lock) {
      vif->vif_bridge_table_lock = vr_zalloc(vr_num_cpus * sizeof(uint8_t),
  ...
  vif->vif_flags =
      (vif->vif_flags & VIF_VR_CAP_MASK) | (req->vifr_flags & ~VIF_VR_CAP_MASK);
...
vr_interface_mirror_md_set(struct vr_interface *vif, vr_interface_req *req)
{
  ...
  if (req->vifr_in_mirror_md_size) {
    if (!vif->vif_in_mirror_md) {
      ...
      vif->vif_in_mirror_md = vr_zalloc(req->vifr_in_mirror_md_size,
    ...
    memcpy(vif->vif_in_mirror_md,
           req->vifr_in_mirror_md,
           req->vifr_in_mirror_md_size);
  ...
  if (req->vifr_out_mirror_md_size) {
    if (!vif->vif_out_mirror_md) {
      ...
      vif->vif_out_mirror_md = vr_zalloc(req->vifr_out_mirror_md_size,
                                         VR_INTERFACE_MIRROR_META_OBJECT);
      if (!vif->vif_out_mirror_md)
        return -ENOMEM;
    ...
    memcpy(vif->vif_out_mirror_md,
           req->vifr_out_mirror_md,
           req->vifr_out_mirror_md_size);
...
vr_interface_change(struct vr_interface *vif, vr_interface_req *req)
{
  ...
  *(ip6 + 1) = req->vifr_ip6_l;
  ...
  return vlan_sub_interface_manage(vif->vif_parent,
                                   vif,
...
vr_interface_add(vr_interface_req *req, bool need_response)
{
...
#ifdef PLATFORM_IS_DPDK
            ret = hif_ops->hif_add_tun_tap(__vrouter_get_interface(router, i),
                                           req);
#endif
...
#ifdef PLATFORM_IS_DPDK
            if (ret) {
              ret = hif_ops->hif_del_tun_tap(
                  __vrouter_get_interface(router, VR_TOTAL_INTERFACES + i));
            }
#else
            /*
  ...
  vif->vif_stats = vr_zalloc(vr_num_cpus * sizeof(struct vr_interface_stats),
  ...
  for (i = 0; i < vr_num_cpus; i++) {
    vif->vif_stats[i].vis_queue_ierrors_to_lcore =
        vr_zalloc(vr_num_cpus * sizeof(uint64_t),
                  VR_INTERFACE_TO_LCORE_ERRORS_OBJECT);
    if (!vif->vif_stats[i].vis_queue_ierrors_to_lcore) {
      ret = -ENOMEM;
      goto error;
   ...
   */
  vif->vif_drop_stats =
      vr_zalloc((VP_DROP_MAX * sizeof(uint64_t)), VR_DROP_STATS_OBJECT);
  if (!vif->vif_drop_stats) {
    ret = -ENOMEM;
    goto error;
  }
  ...
  *(ip6 + 1) = req->vifr_ip6_l;
  ...
  if (req->vifr_hw_queues_size) {
    vif->vif_hw_queues = vr_malloc(sizeof(uint16_t) * req->vifr_hw_queues_size,
  ...
  if (!ret) {
    if (is_vrouter_multihomed(router) &&
      ...
      for (i = 0; i < VR_MAX_PHY_INF; i++) {
        if (router->vr_eth_if[i]) {
          ret =
              hif_ops->hif_add_tun_tap(__vrouter_get_interface(router, i), req);
        }
      }
    }
  }
                            ...
                            struct vr_interface_bond_info *bond_info)
{
  ...
  req->vifr_fab_name = vr_zalloc(req->vifr_fab_name_size * sizeof(uint8_t),
  ...
  if (bond_info->vif_num_slave) {
    for (i = 0; i < bond_info->vif_num_slave; i++) {
      ...
      sl_drv_name_iter +=
          snprintf((buffer_drv + sl_drv_name_iter),
...
vr_vif_stats_reset(struct vr_interface_stats *stats)
{
  ...
  memset(stats->vis_queue_ierrors_to_lcore,
         0,
         (vr_num_cpus * sizeof(stats->vis_queue_ierrors_to_lcore)));
  return;
}

static int
__vr_interface_make_req(vr_interface_req *req,
                        struct vr_interface *intf,
                        unsigned int core)
{
  unsigned int cpu, i;
  uint64_t *ip6;
  ...
  if (intf->vif_src_mac) {
    ...
    for (i = 0; i < VIF_SRC_MACS; i++) {
      ...
      req->vifr_bridge_idx[i] =
          vif_bridge_get_index(intf->vif_parent,
                               intf,
                               intf->vif_src_mac + (i * VR_ETHER_ALEN));
  ...
  if (intf->vif_in_mirror_md_len) {
    memcpy(req->vifr_in_mirror_md,
           intf->vif_in_mirror_md,
  ...
  if (intf->vif_out_mirror_md_len) {
    memcpy(req->vifr_out_mirror_md,
           intf->vif_out_mirror_md,
    ...
    if (hif_ops->hif_get_vlan_info) {
      ...
      if (!hif_ops->hif_get_vlan_info(intf, &vlan_info)) {
        ...
        if (!req->vifr_vlan_name)
          return -ENOMEM;
        snprintf(req->vifr_vlan_name,
                 req->vifr_vlan_name_size,
                 "%s",
                 vlan_info.vlan_name);
      }
    }
  ...
  for (i = 0; i < req->vifr_fat_flow_exclude_ip_list_size; i++) {
    req->vifr_fat_flow_exclude_ip_list[i] =
        (uint64_t)intf->vif_fat_flow_ipv4_exclude_list[i];
  }

  for (i = 0; i < req->vifr_fat_flow_exclude_ip6_u_list_size; i++) {
    req->vifr_fat_flow_exclude_ip6_u_list[i] =
        intf->vif_fat_flow_ipv6_high_exclude_list[i];
    req->vifr_fat_flow_exclude_ip6_l_list[i] =
        intf->vif_fat_flow_ipv6_low_exclude_list[i];
  }
...
static int
vr_interface_make_req(vr_interface_req *req,
                      struct vr_interface *vif,
                      unsigned int core)
{
  ...
  if (vif->vif_fat_flow_ipv4_exclude_list_size) {
    ...
    req->vifr_fat_flow_exclude_ip_list_size =
        vif->vif_fat_flow_ipv4_exclude_list_size;
  }
  ...
  if (vif->vif_fat_flow_ipv6_exclude_list_size) {
    ...
    req->vifr_fat_flow_exclude_ip6_u_list_size =
        vif->vif_fat_flow_ipv6_exclude_list_size;
...
vr_interface_req_get(void)
{
  ...
  req->vifr_src_mac =
      vr_zalloc(VR_ETHER_ALEN * VIF_SRC_MACS, VR_INTERFACE_REQ_MAC_OBJECT);
  if (req->vifr_src_mac)
    req->vifr_src_mac_size = 0;
  ...
  req->vifr_name =
      vr_zalloc(VR_INTERFACE_NAME_LEN, VR_INTERFACE_REQ_NAME_OBJECT);
  ...
  req->vifr_in_mirror_md_size = 0;
  req->vifr_in_mirror_md =
      vr_zalloc(VIF_MAX_MIRROR_MD_SIZE, VR_INTERFACE_REQ_MIRROR_META_OBJECT);
...
vr_interface_dump(vr_interface_req *r)
{
  ...
  for (i = (unsigned int)(r->vifr_marker + 1); i < router->vr_max_interfaces;
       i++) {
    vif = router->vr_interfaces[i];
    if (vif) {
      ...
      if (r->vifr_flags & VIF_FLAG_GET_DROP_STATS) {
        ...
        ret =
            vr_message_dump_object(dumper, VR_DROP_STATS_OBJECT_ID, drop_resp);
        /*
...
exit_get:
  vr_message_response(VR_INTERFACE_OBJECT_ID,
                      ret ? NULL : response,
                      ret,
                      false);
 ...
 */
int
vif_vrf_table_set(struct vr_interface *vif,
                  unsigned int vlan,
                  int vrf,
                  unsigned int nh_id)
{
...
void
__vif_fat_flow_try_free(struct vr_interface *vif,
                        ...
                        unsigned int port_row)
{
  ...
  if (!memcmp(mem_column,
              vif_fat_flow_mem_zero,
...
int
__vif_fat_flow_delete(struct vr_interface *vif,
                      unsigned int proto_index,
                      uint16_t port)
{
  ...
  port_word =
      ((port % VIF_FAT_FLOW_PORTS_PER_BITMAP) * 2) / (sizeof(uint8_t) * 8);
  port_bit =
      ((port % VIF_FAT_FLOW_PORTS_PER_BITMAP) * 2) % (sizeof(uint8_t) * 8);
...
static int
__vif_fat_flow_add_no_prefix_rule(
...
vif_fat_flow_cfg_is_changed(struct vr_interface *vif, vr_interface_req *req)
{
  ...
  for (i = 0; i < vif->fat_flow_cfg_size; i++) {
    ...
    if (memcmp(&new_cfg, &vif->fat_flow_cfg[i], sizeof(vr_fat_flow_cfg_t)) !=
        0) {
      return 1;
    }
  }
...
__vif_fat_flow_alloc_prefix_rule_data(vr_fat_flow_cfg_t *cfg)
{
  ...
  port_data =
      vr_zalloc(sizeof(*port_data), VR_INTERFACE_FAT_FLOW_CONFIG_OBJECT);
  if (!port_data) {
    goto err;
  }
  port_data->second_prefix = NULL;
  /* Add the port data to itable */
  if (vr_itable_set(ptable, cfg->port, (void *)port_data) ==
      VR_ITABLE_ERR_PTR) {
                                       ...
                                       vr_fat_flow_cfg_t *cfg)
{
  ...
  /* Lookup the port */
  port_data =
      (vr_fat_flow_prefix_rule_port_data_t *)vr_itable_get(ptable, cfg->port);
  if (!port_data) {
    port_data =
        vr_zalloc(sizeof(*port_data), VR_INTERFACE_FAT_FLOW_CONFIG_OBJECT);
    if (!port_data) {
...
static int
__vif_fat_flow_add_prefix_rule(struct ip_mtrie *rules,
                               vr_fat_flow_cfg_t *cfg,
...
static int
vif_fat_flow_rules_build(vr_fat_flow_cfg_t *new_cfg,
                         uint16_t new_cfg_size,
                         struct vr_interface *vif)
{
...
vif_fat_flow_add(struct vr_interface *vif, vr_interface_req *req)
{
  ...
  if (!req->vifr_fat_flow_exclude_ip_list_size) {
    memset(vif->vif_fat_flow_ipv4_exclude_list,
           0,
           FAT_FLOW_IPV4_EXCLUDE_LIST_MAX_SIZE * sizeof(uint32_t));
    memset(vif->vif_fat_flow_ipv4_exclude_plen_list,
           0,
           FAT_FLOW_IPV4_EXCLUDE_LIST_MAX_SIZE * sizeof(uint8_t));
    vif->vif_fat_flow_ipv4_exclude_list_size = 0;
  } else {
    if (req->vifr_fat_flow_exclude_ip_list_size >
        FAT_FLOW_IPV4_EXCLUDE_LIST_MAX_SIZE) {
      vr_printf("Vrouter: %s:%d vifr_idx: %d "
                "vifr_fat_flow_exclude_ip_list_size=%d \n",
                __func__,
                __LINE__,
                req->vifr_idx,
                req->vifr_fat_flow_exclude_ip_list_size);
    ...
    for (i = 0; i < req->vifr_fat_flow_exclude_ip_list_size; i++) {
      v4_prefix_len = FAT_FLOW_EXCLUDE_IPV4_PREFIX_LEN(
          req->vifr_fat_flow_exclude_ip_list[i]);
      v4_prefix =
          FAT_FLOW_EXCLUDE_IPV4_PREFIX(req->vifr_fat_flow_exclude_ip_list[i]);
  ...
  if (!req->vifr_fat_flow_exclude_ip6_l_list_size) {
    memset(vif->vif_fat_flow_ipv6_low_exclude_list,
           0,
           FAT_FLOW_IPV6_EXCLUDE_LIST_MAX_SIZE * sizeof(uint64_t));
    memset(vif->vif_fat_flow_ipv6_high_exclude_list,
           0,
           FAT_FLOW_IPV6_EXCLUDE_LIST_MAX_SIZE * sizeof(uint64_t));
    memset(vif->vif_fat_flow_ipv6_exclude_plen_list,
           0,
    ...
    if ((req->vifr_fat_flow_exclude_ip6_l_list_size !=
        ...
        (req->vifr_fat_flow_exclude_ip6_l_list_size >
         FAT_FLOW_IPV6_EXCLUDE_LIST_MAX_SIZE)) {
      vr_printf("Vrouter: %s:%d vifr_idx: %d "
                "vifr_fat_flow_exclude_ip6_l_list_size=%d \n",
                __func__,
                __LINE__,
                req->vifr_idx,
                req->vifr_fat_flow_exclude_ip6_l_list_size);
  ...
  /*
   * Check if fat flow config has changed. If so, then build the new config and
   * rules; Free the old ones.
   */
  if (vif_fat_flow_cfg_is_changed(vif, req)) {
    /* Build the new cfg and rules */
    rc = vif_fat_flow_cfg_build(req,
                                &new_fat_flow_cfg,
                                &new_fat_flow_cfg_size,
                                new_fat_flow_num_rules);
    if (rc < 0) {
      return rc;
    }
...
static uint8_t
vif_fat_flow_port_get(struct vr_interface *vif,
                      uint8_t proto_index,
                      uint16_t port)
{
...
static uint8_t
vif_fat_flow_exclude_list_lookup(struct vr_interface *vif,
                                 unsigned int *saddr,
                                 unsigned int *daddr,
                                 unsigned char *ip6_src,
                                 unsigned char *ip6_dst)
{
  ...
  if (saddr) {
    ...
    for (i = 0; i < vif->vif_fat_flow_ipv4_exclude_list_size; i++) {
      v4_mask = FAT_FLOW_IPV4_PLEN_TO_MASK(
          vif->vif_fat_flow_ipv4_exclude_plen_list[i]);
    ...
    for (i = 0; i < vif->vif_fat_flow_ipv6_exclude_list_size; i++) {
      fat_flow_ipv6_plen_to_mask(vif->vif_fat_flow_ipv6_exclude_plen_list[i],
                                 &v6_mask_h,
                                 &v6_mask_l);
      /* compare src ip */
      memcpy(&v6_prefix_h, (uint8_t *)ip6_src, 8);
      memcpy(&v6_prefix_l, ((uint8_t *)ip6_src) + 8, 8);
      if (((v6_prefix_l & v6_mask_l) ==
           vif->vif_fat_flow_ipv6_low_exclude_list[i]) &&
          ((v6_prefix_h & v6_mask_h) ==
           vif->vif_fat_flow_ipv6_high_exclude_list[i])) {
        return 1;
      }
      /* compare dst ip */
...
static int
__vif_fat_flow_ip_prefix_lookup(struct ip_mtrie *mtrie,
                                unsigned int *ip,
                                unsigned char *ip6,
                                uint8_t *aggr_mask)
{
...
static int
vif_fat_flow_prefix_proto_port_match(uint8_t rule_type,
                                     int incoming_vif,
                                     vr_fat_flow_prefix_rule_data_t *rdata,
                                     unsigned int *dip,
                                     unsigned char *dip6,
                                     ...
                                     uint16_t *fat_flow_mask)
{
  ...
  if (ptable) {
    ...
    src_port_info =
        (vr_fat_flow_prefix_rule_port_data_t *)vr_itable_get(ptable, sport);
    if (src_port_info && (!(src_port_info->rule_type & rule_type))) {
      src_port_info = NULL;
    }

    dst_port_info =
        (vr_fat_flow_prefix_rule_port_data_t *)vr_itable_get(ptable, dport);
    if (dst_port_info && (!(dst_port_info->rule_type & rule_type))) {
      dst_port_info = NULL;
    }

    zero_port_info =
        (vr_fat_flow_prefix_rule_port_data_t *)vr_itable_get(ptable, 0);
...
static uint16_t
vif_fat_flow_prefix_rule_match(int incoming_vif,
                               struct vr_interface *vif,
                               uint8_t proto,
                               uint16_t sport,
                               uint16_t dport,
...
uint16_t
vif_fat_flow_lookup(int incoming_vif,
                    struct vr_interface *vif,
                    uint8_t proto,
                    uint16_t sport,
                    uint16_t dport,
                    ...
                    unsigned char *ip6_dst)
{
  ...
  return vif_fat_flow_prefix_rule_match(incoming_vif,
                                        vif,
                                        proto,
                                        h_sport,
                                        h_dport,
...
int
vr_gro_vif_add(struct vrouter *router,
               unsigned int os_idx,
               char *name,
               unsigned short idx)
{
