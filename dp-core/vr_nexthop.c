...
static int __vrouter_set_nexthop(struct vrouter* router, unsigned int index, struct vr_nexthop* nh)
{
...
static void vrouter_free_nexthop(struct vr_nexthop* nh)
{
    ...
    else if ((nh->nh_type == NH_TUNNEL) && (nh->nh_flags & NH_FLAG_TUNNEL_UDP) && (nh->nh_family == AF_INET6))
    {
...
static int nh_tunnel_loop_detect_handle(struct vr_packet* pkt, struct vr_nexthop* nh, struct vr_forwarding_md* fmd,
                                        uint32_t dip)
{
    if (!pkt || !nh || !fmd || !dip)
        return 0;
...
struct vr_interface* nh_underlay_tunnel_vif_selection(struct vr_packet* pkt, struct vr_forwarding_md* fmd,
                                                      struct vr_nexthop* nh, unsigned short* drop_reason)
{
...
static nh_processing_t nh_resolve(struct vr_packet* pkt, struct vr_nexthop* nh, struct vr_forwarding_md* fmd)
{
...
static nh_processing_t nh_vrf_translate(struct vr_packet* pkt, struct vr_nexthop* nh, struct vr_forwarding_md* fmd)
{
    struct vr_vrf_stats* stats = NULL;
...
static nh_processing_t nh_l2_rcv(struct vr_packet* pkt, struct vr_nexthop* nh, struct vr_forwarding_md* fmd)
{
...
static nh_processing_t nh_l3_rcv(struct vr_packet* pkt, struct vr_nexthop* nh, struct vr_forwarding_md* fmd)
{
    struct vr_vrf_stats* stats = NULL;
...
static int nh_push_mpls_header(struct vr_packet* pkt, unsigned int label, struct vr_forwarding_class_qos* qos,
 ...
 */
static bool nh_udp_tunnel_helper(struct vr_packet* pkt, unsigned short sport, unsigned short dport, unsigned int sip,
...
static bool nh_udp_tunnel6_helper(struct vr_packet* pkt, struct vr_nexthop* nh, uint8_t* sip, uint16_t sport,
                                  uint16_t dport)
{
    struct vr_ip6* ip6;
    struct vr_udp* udp;
...
static bool nh_pbb_tunnel_helper(struct vrouter* router, struct vr_packet** pkt, struct vr_forwarding_md* fmd,
                                 uint8_t* dmac, uint8_t* smac, uint32_t isid)
{
    int pbb_head_space;
...
static bool nh_vxlan_tunnel_helper(struct vrouter* router, struct vr_packet** pkt, struct vr_forwarding_md* fmd,
                                   unsigned int sip, unsigned int dip)
{
    ...
    return nh_udp_tunnel_helper(*pkt, htons(udp_src_port), htons(VR_VXLAN_UDP_DST_PORT), sip, dip, qos);
}
...
static int nh_composite_ecmp_validate_src(struct vr_packet* pkt, struct vr_nexthop* nh, struct vr_forwarding_md* fmd,
                                          void* ret_data)
{
    int i;
     ...
     */
    if (!cnh || (!cnh->nh_validate_src) || (NH_SOURCE_INVALID == cnh->nh_validate_src(pkt, cnh, fmd, NULL)))
    {
 ...
 */
static int nh_composite_ecmp_select_by_rflow(struct vr_packet* pkt, struct vr_nexthop* nh, struct vr_forwarding_md* fmd,
...
static int nh_composite_ecmp_select_nh(struct vr_packet* pkt, struct vr_nexthop* nh, struct vr_forwarding_md* fmd)
{
    ...
    if (!fe)
    {
        ...
        if (pkt->vp_type == VP_TYPE_IP)
        {
            ...
            switch (ret)
            {
                case 0:
                     ...
                     * - If this is a head fragment, clone it and enqueue it to the
                     * assembler which would result in flushing of non-head fragments which
        ...
        else if (pkt->vp_type == VP_TYPE_IP6)
        {
            ...
            ret = vr_inet6_get_flow_key(nh->nh_router, fmd->fmd_dvrf, pkt, fmd->fmd_vlan, flowp, hash, 0);
            switch (ret)
            {
                case 0:
...
static nh_processing_t nh_composite_ecmp(struct vr_packet* pkt, struct vr_nexthop* nh, struct vr_forwarding_md* fmd)
{
    ...
    {
        vr_fmd_set_label(fmd, nh->nh_component_nh[fmd->fmd_ecmp_nh_index].cnh_label, VR_LABEL_TYPE_UNKNOWN);
    }
...
static int nh_composite_mcast_validate_src(struct vr_packet* pkt, struct vr_nexthop* nh, struct vr_forwarding_md* fmd,
                                           void* ret_flags)
{
    int i, j;
    ...
    for (j = 0; j < nh->nh_component_cnt; j++)
    {
        ...
        if (fabric_nh->nh_type != NH_COMPOSITE ||
            !(fabric_nh->nh_flags & (NH_FLAG_COMPOSITE_FABRIC | NH_FLAG_COMPOSITE_EVPN | NH_FLAG_COMPOSITE_TOR)))
            continue;
...
static int nh_handle_unknown_unicast(struct vr_packet* pkt, struct vr_eth* eth, struct vr_forwarding_md* fmd,
                                     unsigned int pkt_src)
{
...
static int nh_handle_mcast_control_pkt(struct vr_packet* pkt, struct vr_eth* eth, struct vr_forwarding_md* fmd,
...
static nh_processing_t nh_composite_mcast(struct vr_packet* pkt, struct vr_nexthop* nh, struct vr_forwarding_md* fmd)
{
    int i, clone_size;
...
static nh_processing_t nh_composite_encap(struct vr_packet* pkt, struct vr_nexthop* nh, struct vr_forwarding_md* fmd)
{
    int i, j;
    struct vr_vrf_stats* stats = NULL;
    struct vr_nexthop* dir_nh;
...
static nh_processing_t nh_composite_tor(struct vr_packet* pkt, struct vr_nexthop* nh, struct vr_forwarding_md* fmd)
{
    int i, j;
    struct vr_vrf_stats* stats = NULL;
    struct vr_nexthop* dir_nh;
...
static nh_processing_t nh_composite_evpn(struct vr_packet* pkt, struct vr_nexthop* nh, struct vr_forwarding_md* fmd)
{
    int i, j;
...
static nh_processing_t nh_composite_fabric(struct vr_packet* pkt, struct vr_nexthop* nh, struct vr_forwarding_md* fmd)
{
    int i, j, flag = 0;
    int32_t label;
    ...
    for (i = 0; i < nh->nh_component_cnt; i++)
    {
        ...
        if (vif_is_virtual(new_pkt->vp_if) || (fmd->fmd_src == TOR_SOURCE))
        {
            ...
            if (nh->nh_family == AF_BRIDGE)
            {
                ...
                if (nh_vxlan_tunnel_helper(nh->nh_router, &new_pkt, fmd, sip, dip) == false)
                {
                    PKT_LOG(VP_DROP_PUSH, pkt, 0, VR_NEXTHOP_C, __LINE__);
                    vr_pfree(new_pkt, VP_DROP_PUSH);
                    break;
...
nh_processing_t nh_discard(struct vr_packet* pkt, struct vr_nexthop* nh, struct vr_forwarding_md* fmd)
{
    struct vr_vrf_stats* stats = NULL;
...
static uint8_t* nh_generate_mirroring_sip(struct vr_nexthop* nh, struct vr_packet* pkt, struct vr_forwarding_md* fmd)
{
    uint16_t intf_id;
    mirror_type_t mtype;
    struct vr_interface* vif = NULL;
...
static nh_processing_t nh_udp_tunnel(struct vr_packet* pkt, struct vr_nexthop* nh, struct vr_forwarding_md* fmd)
{
    int ret = -1;
    uint8_t* vif_ip = NULL;
    uint16_t sport = 0;
    ...
    if (pkt->vp_type == VP_TYPE_IP)
    {
        ret = vr_inet_get_flow_key(nh->nh_router, pkt, fmd, flowp, VR_FLOW_KEY_ALL, VR_FRAG_FLAG_MIRROR);
    }
    else if (pkt->vp_type == VP_TYPE_IP6)
    {
        ret = vr_inet6_get_flow_key(nh->nh_router, fmd->fmd_dvrf, pkt, fmd->fmd_vlan, flowp, VR_FLOW_KEY_ALL,
                                    VR_FRAG_FLAG_MIRROR);
    }
    ...
    if (nh->nh_family == AF_INET)
    {
        ...
        if (nh_udp_tunnel_helper(pkt, htons(sport), nh->nh_udp_tun_dport, sip, nh->nh_udp_tun_dip, qos) == false)
        {
            PKT_LOG(VP_DROP_PUSH, pkt, flowp, VR_NEXTHOP_C, __LINE__);
            goto send_fail;
        }
 ...
 */
static nh_processing_t nh_vxlan_tunnel(struct vr_packet* pkt, struct vr_nexthop* nh, struct vr_forwarding_md* fmd)
{
    ...
    if ((pkt->vp_type == VP_TYPE_IP) || (pkt->vp_type == VP_TYPE_IP6))
    {
        if (vr_has_to_fragment(vif, pkt, overhead_len) && vr_ip_dont_fragment_set(pkt))
        {
            ...
            trap_arg.df_mtu = vif_get_mtu(vif) - (overhead_len + pkt_get_network_header_off(pkt) - pkt->vp_data);
    ...
    if (nh_vxlan_tunnel_helper(nh->nh_router, &pkt, fmd, nh->nh_vxlan_tun_sip, nh->nh_vxlan_tun_dip) == false)
        goto send_fail;
    ...
    {
        if (vif->vif_set_rewrite(vif, &pkt, fmd, (nh->nh_data + (nh->nh_vxlan_tun_encap_len * vif->vif_idx)),
    ...
    {
        if (vif->vif_set_rewrite(vif, &pkt, fmd, nh->nh_data, nh->nh_vxlan_tun_encap_len) < 0)
        {
            goto send_fail;
        }
    }
...
static nh_processing_t nh_pbb_tunnel(struct vr_packet* pkt, struct vr_nexthop* nh, struct vr_forwarding_md* fmd)
{
    struct vr_vrf_stats* stats = NULL;
    ...
    if (!nh_pbb_tunnel_helper(nh->nh_router, &pkt, fmd, nh->nh_pbb_mac, pkt->vp_if->vif_pbb_mac, pkt->vp_if->vif_isid))
    {
        PKT_LOG(VP_DROP_PUSH, pkt, 0, VR_NEXTHOP_C, __LINE__);
        vr_pfree(pkt, VP_DROP_PUSH);
        return NH_PROCESSING_COMPLETE;
...
static int nh_pbb_tunnel_validate_src(struct vr_packet* pkt, struct vr_nexthop* nh, struct vr_forwarding_md* fmd,
                                      void* ret_data)
{
    bool root = false;
...
static int nh_mpls_udp_tunnel_validate_src(struct vr_packet* pkt, struct vr_nexthop* nh, struct vr_forwarding_md* fmd,
                                           void* ret_data)
{
...
static int nh_vxlan_tunnel_validate_src(struct vr_packet* pkt, struct vr_nexthop* nh, struct vr_forwarding_md* fmd,
                                        void* ret_data)
{
    if (fmd->fmd_outer_src_ip == nh->nh_udp_tun_dip)
        return NH_SOURCE_VALID;
 ...
 */
static nh_processing_t nh_mpls_udp_tunnel(struct vr_packet* pkt, struct vr_nexthop* nh, struct vr_forwarding_md* fmd)
{
    ...
    if (nh_udp_tunnel_helper(pkt, htons(udp_src_port), htons(VR_MPLS_OVER_UDP_DST_PORT), tun_sip, tun_dip, qos) ==
        false)
    {
        PKT_LOG(reason, pkt, 0, VR_NEXTHOP_C, __LINE__);
        goto send_fail;
    ...
    {
        tun_encap_rewrite =
            vif->vif_set_rewrite(vif, &pkt, fmd, (nh->nh_data + (tun_encap_len * vif->vif_idx)), tun_encap_len);
    }
    else
    {
...
static int nh_gre_tunnel_validate_src(struct vr_packet* pkt, struct vr_nexthop* nh, struct vr_forwarding_md* fmd,
                                      void* ret_data)
{
...
static nh_processing_t nh_gre_tunnel(struct vr_packet* pkt, struct vr_nexthop* nh, struct vr_forwarding_md* fmd)
{
    ...
    if (vif_is_fabric(vif) && (nh->nh_flags & NH_FLAG_TUNNEL_UNDERLAY_ECMP))
    {
        tun_encap_rewrite = vif->vif_set_rewrite(
            vif, &pkt, fmd, (nh->nh_data + (nh->nh_gre_tun_encap_len * vif->vif_idx)), nh->nh_gre_tun_encap_len);
    }
    else
    {
...
static nh_processing_t nh_encap_l2(struct vr_packet* pkt, struct vr_nexthop* nh, struct vr_forwarding_md* fmd)
{
...
static int nh_encap_validate_src(struct vr_packet* pkt, struct vr_nexthop* nh, struct vr_forwarding_md* fmd,
                                 void* ret_data)
{
    bool root = false;
...
static nh_processing_t nh_encap_l3_mcast(struct vr_packet* pkt, struct vr_nexthop* nh, struct vr_forwarding_md* fmd)
{
    uint8_t* ptr;
...
static nh_processing_t nh_encap_l3(struct vr_packet* pkt, struct vr_nexthop* nh, struct vr_forwarding_md* fmd)
{
    int rewrite_len;
...
static int nh_composite_mcast_validate(struct vr_component_nh* component_nh, vr_nexthop_req* req)
{
    ...
    if (req->nhr_flags & (NH_FLAG_COMPOSITE_FABRIC | NH_FLAG_COMPOSITE_EVPN | NH_FLAG_COMPOSITE_TOR))
    {
        if (!l3_seen && !l2_seen)
            return -1;
    ...
    if (req->nhr_flags & NH_FLAG_MCAST)
    {
        ...
        for (i = 0; i < req->nhr_nh_list_size; i++)
        {
            ...
            if (!(tmp_nh->nh_flags & (NH_FLAG_COMPOSITE_FABRIC | NH_FLAG_COMPOSITE_EVPN | NH_FLAG_COMPOSITE_TOR |
...
static int nh_composite_add(struct vr_nexthop* nh, vr_nexthop_req* req)
{
    ...
    {
        component_nh = vr_zalloc(req->nhr_nh_list_size * sizeof(struct vr_component_nh), VR_NEXTHOP_COMPONENT_OBJECT);
        if (!component_nh)
        {
            ret = -ENOMEM;
            goto exit_add;
        ...
        if (req->nhr_flags & NH_FLAG_COMPOSITE_ECMP)
        {
            if (active)
            {
                component_ecmp = vr_zalloc(active * sizeof(struct vr_component_nh), VR_NEXTHOP_COMPONENT_OBJECT);
                if (!component_ecmp)
                {
                    ret = -ENOMEM;
                    goto exit_add;
...
static int nh_tunnel_add(struct vr_nexthop* nh, vr_nexthop_req* req)
{
    ...
    for (i = 0; i < VR_MAX_PHY_INF; i++) old_vif[i] = nh->nh_dev_arr[i];
    ...
    if (nh->nh_flags & NH_FLAG_TUNNEL_UNDERLAY_ECMP)
    {
        for (i = 0; i < VR_MAX_PHY_INF; i++)
        {
            ...
            if (nh->nh_encap_valid[i])
            {
                vif[i] = vrouter_get_interface(nh->nh_rid, req->nhr_encap_oif_id[i]);
                nh->nh_valid_underlay_dev[nh->nh_valid_underlay_dev_count] = vif[i];
...
unsigned int vr_nexthop_req_get_size(void* req_p)
{
    ...
    if ((req->nhr_type == NH_TUNNEL) && (req->nhr_flags & NH_FLAG_TUNNEL_UDP) && (req->nhr_family == AF_INET6))
...
static int vr_nexthop_make_req(vr_nexthop_req* req, struct vr_nexthop* nh)
{
    ...
    if ((nh->nh_flags & NH_FLAG_INDIRECT) && (cnh = nh->nh_direct_nh))
    {
        ...
        req->nhr_nh_list = vr_zalloc(req->nhr_nh_list_size * sizeof(unsigned int), VR_NEXTHOP_REQ_LIST_OBJECT);
        if (!req->nhr_nh_list)
            return -ENOMEM;
...
static vr_nexthop_req* vr_nexthop_req_get(struct vr_nexthop* nh)
{
    ...
    if (nh->nh_flags & NH_FLAG_TUNNEL_UNDERLAY_ECMP)
    {
        ...
        nhr->nhr_encap_oif_id =
            vr_zalloc(nhr->nhr_encap_oif_id_size * sizeof(unsigned int), VR_NEXTHOP_REQ_LIST_OBJECT);
        if (!nhr->nhr_encap_oif_id)
            goto fail;

        nhr->nhr_encap_valid_size = VR_MAX_PHY_INF;
        nhr->nhr_encap_valid = vr_zalloc(nhr->nhr_encap_valid_size * sizeof(unsigned int), VR_NEXTHOP_REQ_LIST_OBJECT);
    ...
    if ((nh->nh_type == NH_TUNNEL) && (nh->nh_flags & NH_FLAG_TUNNEL_UDP) && (nh->nh_family == AF_INET6))
    {
...
fail:
    if (nhr->nhr_encap_oif_id)
    {
        vr_free(nhr->nhr_encap_oif_id, nhr->nhr_encap_oif_id_size * sizeof(unsigned int));
        nhr->nhr_encap_oif_id = NULL;
    }

    if (nhr->nhr_encap_valid)
    {
        vr_free(nhr->nhr_encap_valid, nhr->nhr_encap_valid_size * sizeof(unsigned int));
        nhr->nhr_encap_valid = NULL;
    }
...
int vr_is_local_ecmp_nh(struct vr_nexthop* nh)
{
    ...
    for (i = 0; i < nh->nh_component_cnt; i++)
    {
        if (nh->nh_component_nh && nh->nh_component_nh[i].cnh && nh->nh_component_nh[i].cnh->nh_type != NH_ENCAP)
        {
            return 0;
        }
    }
...
int vr_nexthop_init(struct vrouter* router)
{
...
int vr_nexthop_update_offload_vrfstats(uint32_t vrfid, uint32_t num_cntrs, uint64_t* cntrs)
{
    ...
    if (stats && num_cntrs <= sizeof(struct vr_vrf_stats) / sizeof(uint64_t))
    {
        ...
        while (num_cntrs-- > 0) *dst_cntr++ += *cntrs++;
    }
