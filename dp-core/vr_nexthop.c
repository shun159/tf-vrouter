...
extern bool
vr_has_to_fragment(struct vr_interface *, struct vr_packet *, unsigned int);
...
static int
__vrouter_set_nexthop(struct vrouter *router,
                      unsigned int index,
                      struct vr_nexthop *nh)
{
...
static int
nh_tunnel_loop_detect_handle(struct vr_packet *pkt,
                             struct vr_nexthop *nh,
...
struct vr_interface *
nh_underlay_tunnel_vif_selection(struct vr_packet *pkt,
                                 struct vr_forwarding_md *fmd,
                                 struct vr_nexthop *nh,
...
nh_resolve(struct vr_packet *pkt,
           struct vr_nexthop *nh,
           struct vr_forwarding_md *fmd)
{
...
static nh_processing_t
nh_vrf_translate(struct vr_packet *pkt,
                 struct vr_nexthop *nh,
                 struct vr_forwarding_md *fmd)
{
  struct vr_vrf_stats *stats = NULL;
...
static nh_processing_t
nh_l2_rcv(struct vr_packet *pkt,
          struct vr_nexthop *nh,
          struct vr_forwarding_md *fmd)
{
...
static nh_processing_t
nh_l3_rcv(struct vr_packet *pkt,
          struct vr_nexthop *nh,
          struct vr_forwarding_md *fmd)
{
  struct vr_vrf_stats *stats = NULL;
...
static int
nh_push_mpls_header(struct vr_packet *pkt,
                    unsigned int label,
                    ...
                    bool is_bottom_label)
{
  ...
  *lbl =
      htonl((label << VR_MPLS_LABEL_SHIFT) | exp_qos | (label_pos << 8) | ttl);
 ...
 */
static bool
nh_udp_tunnel_helper(struct vr_packet *pkt,
...
static bool
nh_pbb_tunnel_helper(struct vrouter *router,
...
static bool
nh_vxlan_tunnel_helper(struct vrouter *router,
                       struct vr_packet **pkt,
                       struct vr_forwarding_md *fmd,
                       unsigned int sip,
                       unsigned int dip)
{
  ...
  if (pkt_head_space(*pkt) < VR_VXLAN_HDR_LEN) {
    expanded_pkt =
        vr_pexpand_head(*pkt, VR_VXLAN_HDR_LEN - pkt_head_space(*pkt));
    if (!expanded_pkt) {
      return false;
    }
    *pkt = expanded_pkt;
  ...
  return nh_udp_tunnel_helper(*pkt,
                              htons(udp_src_port),
...
static int
nh_composite_ecmp_validate_src(struct vr_packet *pkt,
                               struct vr_nexthop *nh,
                               struct vr_forwarding_md *fmd,
                               void *ret_data)
{
...
static int
nh_composite_ecmp_select_nh(struct vr_packet *pkt,
                            struct vr_nexthop *nh,
                            struct vr_forwarding_md *fmd)
{
    ...
    if (fe) {
      ...
      if (fmd->fmd_ecmp_nh_index == -1) {
        ...
        if (rflow_src_info != (unsigned int)-1) {
          ecmp_index =
              nh_composite_ecmp_select_by_rflow(pkt, nh, fmd, rflow_src_info);
        }
      }
    }
  }
  ...
  if (!fe) {
    ...
    if (pkt->vp_type == VP_TYPE_IP) {
      ...
      ret = vr_inet_get_flow_key(nh->nh_router,
                                 pkt,
                                 fmd,
                                 flowp,
                                 hash,
      ...
      ret = vr_inet6_get_flow_key(nh->nh_router,
                                  fmd->fmd_dvrf,
                                  pkt,
                                  fmd->fmd_vlan,
                                  flowp,
...
static nh_processing_t
nh_composite_ecmp(struct vr_packet *pkt,
                  struct vr_nexthop *nh,
                  struct vr_forwarding_md *fmd)
{
...
static int
nh_composite_mcast_validate_src(struct vr_packet *pkt,
                                struct vr_nexthop *nh,
                                struct vr_forwarding_md *fmd,
                                void *ret_flags)
{
...
static int
nh_handle_unknown_unicast(struct vr_packet *pkt,
                          struct vr_eth *eth,
                          struct vr_forwarding_md *fmd,
...
static int
nh_handle_mcast_control_pkt(struct vr_packet *pkt,
                            struct vr_eth *eth,
                            struct vr_forwarding_md *fmd,
                            unsigned int pkt_src,
                            bool *flood_to_vms)
...
static nh_processing_t
nh_composite_mcast(struct vr_packet *pkt,
                   struct vr_nexthop *nh,
                   struct vr_forwarding_md *fmd)
{
  int i, clone_size;
  ...
  if (nh->nh_family == AF_BRIDGE) {
    ...
    handled =
        nh_handle_mcast_control_pkt(pkt, eth, fmd, pkt_src, &flood_to_vms);
    if (handled)
      return NH_PROCESSING_COMPLETE;
    ...
    if (!fmd->fmd_udp_src_port) {
      ...
      fmd->fmd_udp_src_port =
          (uint16_t)(((uint64_t)hashval * port_range) >> 32);
...
static nh_processing_t
nh_composite_encap(struct vr_packet *pkt,
                   struct vr_nexthop *nh,
                   struct vr_forwarding_md *fmd)
{
  int i, j;
...
static nh_processing_t
nh_composite_tor(struct vr_packet *pkt,
                 struct vr_nexthop *nh,
                 struct vr_forwarding_md *fmd)
{
  int i, j;
  ...
  for (i = 0; i < nh->nh_component_cnt; i++) {
    ...
    if (fmd->fmd_outer_src_ip &&
        fmd->fmd_outer_src_ip == dir_nh->nh_udp_tun_dip)
      continue;
...
static nh_processing_t
nh_composite_evpn(struct vr_packet *pkt,
                  struct vr_nexthop *nh,
                  struct vr_forwarding_md *fmd)
{
  int i, j;
  ...
  for (i = 0; i < nh->nh_component_cnt; i++) {
    ...
    if (nh->nh_flags & NH_FLAG_TUNNEL_PBB) {
      if (!nh_pbb_tunnel_helper(nh->nh_router,
                                &new_pkt,
                                fmd,
                                eth_mac,
                                pkt->vp_if->vif_pbb_mac,
                                pkt->vp_if->vif_isid)) {
...
static nh_processing_t
nh_composite_fabric(struct vr_packet *pkt,
                    struct vr_nexthop *nh,
                    struct vr_forwarding_md *fmd)
{
  int i, j, flag = 0;
  ...
  for (i = 0; i < nh->nh_component_cnt; i++) {
    ...
    if (vif_is_virtual(new_pkt->vp_if) || (fmd->fmd_src == TOR_SOURCE)) {
      ...
      if (nh->nh_family == AF_BRIDGE) {
        ...
        if (nh_vxlan_tunnel_helper(nh->nh_router, &new_pkt, fmd, sip, dip) ==
            false) {
          PKT_LOG(VP_DROP_PUSH, pkt, 0, VR_NEXTHOP_C, __LINE__);
          vr_pfree(new_pkt, VP_DROP_PUSH);
          break;
...
nh_processing_t
nh_discard(struct vr_packet *pkt,
           struct vr_nexthop *nh,
           struct vr_forwarding_md *fmd)
{
  struct vr_vrf_stats *stats = NULL;
...
static nh_processing_t
nh_udp_tunnel(struct vr_packet *pkt,
              struct vr_nexthop *nh,
              struct vr_forwarding_md *fmd)
{
  int ret = -1;
 ...
 */
static nh_processing_t
nh_vxlan_tunnel(struct vr_packet *pkt,
                struct vr_nexthop *nh,
                struct vr_forwarding_md *fmd)
{
  ...
  if (nh_vxlan_tunnel_helper(nh->nh_router,
                             &pkt,
                             fmd,
                             nh->nh_vxlan_tun_sip,
  ...
  if (pkt_head_space(pkt) < nh->nh_vxlan_tun_encap_len) {
    tmp_pkt =
        vr_pexpand_head(pkt, nh->nh_vxlan_tun_encap_len - pkt_head_space(pkt));
    if (!tmp_pkt) {
      goto send_fail;
    }
    pkt = tmp_pkt;
...
static nh_processing_t
nh_pbb_tunnel(struct vr_packet *pkt,
              struct vr_nexthop *nh,
              struct vr_forwarding_md *fmd)
{
  struct vr_vrf_stats *stats = NULL;
...
static int
nh_pbb_tunnel_validate_src(struct vr_packet *pkt,
                           struct vr_nexthop *nh,
                           struct vr_forwarding_md *fmd,
                           void *ret_data)
{
...
static int
nh_mpls_udp_tunnel_validate_src(struct vr_packet *pkt,
                                struct vr_nexthop *nh,
                                struct vr_forwarding_md *fmd,
                                void *ret_data)
{
...
static int
nh_vxlan_tunnel_validate_src(struct vr_packet *pkt,
                             struct vr_nexthop *nh,
                             struct vr_forwarding_md *fmd,
                             void *ret_data)
{
 ...
 */
static nh_processing_t
nh_mpls_udp_tunnel(struct vr_packet *pkt,
                   struct vr_nexthop *nh,
                   struct vr_forwarding_md *fmd)
{
  ...
  mudp_head_space = (label_count * VR_MPLS_HDR_LEN) + sizeof(struct vr_ip) +
  ...
  if (vif_is_fabric(vif) && (nh->nh_flags & NH_FLAG_TUNNEL_UNDERLAY_ECMP)) {
    tun_encap_rewrite =
        vif->vif_set_rewrite(vif,
                             &pkt,
                             fmd,
                             (nh->nh_data + (tun_encap_len * vif->vif_idx)),
                             tun_encap_len);
  } else {
    tun_encap_rewrite =
        vif->vif_set_rewrite(vif, &pkt, fmd, nh->nh_data, tun_encap_len);
  }
...
static int
nh_gre_tunnel_validate_src(struct vr_packet *pkt,
                           struct vr_nexthop *nh,
                           struct vr_forwarding_md *fmd,
                           void *ret_data)
{
...
static nh_processing_t
nh_gre_tunnel(struct vr_packet *pkt,
              struct vr_nexthop *nh,
              struct vr_forwarding_md *fmd)
{
  ...
  if (vif_is_fabric(vif) && (nh->nh_flags & NH_FLAG_TUNNEL_UNDERLAY_ECMP)) {
    ...
    tun_encap_rewrite = vif->vif_set_rewrite(vif,
                                             &pkt,
                                             fmd,
                                             nh->nh_data,
                                             nh->nh_gre_tun_encap_len);
 ...
 */
int
nh_output(struct vr_packet *pkt,
          struct vr_nexthop *nh,
          struct vr_forwarding_md *fmd)
{
...
static nh_processing_t
nh_encap_l2(struct vr_packet *pkt,
            struct vr_nexthop *nh,
            struct vr_forwarding_md *fmd)
{
  ...
  if (vr_fmd_etree_is_enabled(fmd)) {
    if ((!vr_fmd_etree_is_root(fmd)) &&
        (!(nh->nh_flags & NH_FLAG_ETREE_ROOT))) {
      fprintf(stderr, "Drop: LEAF_TO_LEAF\n");
      PKT_LOG(VP_DROP_LEAF_TO_LEAF, pkt, 0, VR_NEXTHOP_C, __LINE__);
      vr_pfree(pkt, VP_DROP_LEAF_TO_LEAF);
      return NH_PROCESSING_COMPLETE;
    }
  ...
  if (vr_pkt_is_gro(pkt) && vif_is_virtual(vif) &&
    ...
    if (vr_gro_input(pkt, nh)) {
      fprintf(stderr, "Packet absorbed by GRO\n");
      if (stats)
        stats->vrf_gros++;
      return NH_PROCESSING_COMPLETE;
    }
...
static int
nh_encap_validate_src(struct vr_packet *pkt,
                      struct vr_nexthop *nh,
                      struct vr_forwarding_md *fmd,
                      void *ret_data)
{
...
static nh_processing_t
nh_encap_l3_mcast(struct vr_packet *pkt,
                  struct vr_nexthop *nh,
                  struct vr_forwarding_md *fmd)
{
  uint8_t *ptr;
...
static nh_processing_t
nh_encap_l3(struct vr_packet *pkt,
            struct vr_nexthop *nh,
            struct vr_forwarding_md *fmd)
{
  int rewrite_len;
  ...
  rewrite_len =
      vif->vif_set_rewrite(vif, &pkt, fmd, nh->nh_data, nh->nh_encap_len);
  if (rewrite_len < 0) {
...
nh_composite_add(struct vr_nexthop *nh, vr_nexthop_req *req)
{
  ...
  if (req->nhr_nh_list_size) {
    component_nh =
        vr_zalloc(req->nhr_nh_list_size * sizeof(struct vr_component_nh),
    ...
    for (i = 0; i < req->nhr_nh_list_size; i++) {
      component_nh[i].cnh =
          vrouter_get_nexthop(req->nhr_rid, req->nhr_nh_list[i]);
    ...
    if (req->nhr_flags & NH_FLAG_COMPOSITE_ECMP) {
      if (active) {
        component_ecmp = vr_zalloc(active * sizeof(struct vr_component_nh),
                                   VR_NEXTHOP_COMPONENT_OBJECT);
        if (!component_ecmp) {
          ret = -ENOMEM;
          goto exit_add;
...
nh_tunnel_add(struct vr_nexthop *nh, vr_nexthop_req *req)
{
  ...
  if (nh->nh_flags & NH_FLAG_TUNNEL_GRE) {
    ...
    if (req->nhr_family == AF_INET) {
      ...
      if (!nh->nh_udp_tun6_sip) {
        nh->nh_udp_tun6_sip =
            vr_malloc(VR_IP6_ADDRESS_LEN, VR_NETWORK_ADDRESS_OBJECT);
        if (!nh->nh_udp_tun6_sip) {
          ret = -ENOMEM;
          goto exit_error;
        }
...
vr_nexthop_make_req(vr_nexthop_req *req, struct vr_nexthop *nh)
{
  ...
  case NH_COMPOSITE:
    ...
    req->nhr_ecmp_config_hash =
        nh->nh_ecmp_config_hash & NH_ECMP_CONFIG_HASH_MASK;
  ...
  case NH_TUNNEL:
    ...
    if (nh->nh_flags & NH_FLAG_TUNNEL_GRE) {
      ...
      if (nh->nh_flags & NH_FLAG_L3_VXLAN) {
        ...
        req->nhr_rw_dst_mac =
            vr_zalloc(req->nhr_rw_dst_mac_size, VR_NEXTHOP_REQ_BMAC_OBJECT);
      ...
      req->nhr_pbb_mac =
          vr_zalloc(req->nhr_pbb_mac_size, VR_NEXTHOP_REQ_BMAC_OBJECT);
      if (!req->nhr_pbb_mac)
        return -ENOMEM;
  ...
  if (req->nhr_encap_size) {
    ...
    req->nhr_encap =
        vr_zalloc(req->nhr_encap_size, VR_NEXTHOP_REQ_ENCAP_OBJECT);
    if (req->nhr_encap) {
...
vr_nexthop_req_get(struct vr_nexthop *nh)
{
  ...
  if (nh->nh_flags & NH_FLAG_TUNNEL_UNDERLAY_ECMP) {
    ...
    nhr->nhr_encap_oif_id =
        vr_zalloc(nhr->nhr_encap_oif_id_size * sizeof(unsigned int),
                  VR_NEXTHOP_REQ_LIST_OBJECT);
    ...
    nhr->nhr_encap_valid =
        vr_zalloc(nhr->nhr_encap_valid_size * sizeof(unsigned int),
                  VR_NEXTHOP_REQ_LIST_OBJECT);
...
fail:
  if (nhr->nhr_encap_oif_id) {
    vr_free(nhr->nhr_encap_oif_id,
            nhr->nhr_encap_oif_id_size * sizeof(unsigned int));
    nhr->nhr_encap_oif_id = NULL;
  }

  if (nhr->nhr_encap_valid) {
    vr_free(nhr->nhr_encap_valid,
            nhr->nhr_encap_valid_size * sizeof(unsigned int));
    nhr->nhr_encap_valid = NULL;
  }
...
vr_nexthop_dump(vr_nexthop_req *r)
{
  ...
  for (i = (unsigned int)(r->nhr_marker + 1); i < router->vr_max_nexthops;
       i++) {
    ...
    if (nh) {
      ...
      if ((ret < 0) ||
          ((ret = vr_message_dump_object(dumper, VR_NEXTHOP_OBJECT_ID, resp)) <=
           0)) {
        vr_nexthop_req_destroy(resp);
        break;
      }
...
vr_get_ecmp_first_member_dev(struct vr_nexthop *nh)
{
  ...
  if (nh->nh_component_nh)
    return nh->nh_component_nh[0].cnh ? nh->nh_component_nh[0].cnh->nh_dev
                                      : NULL;
  else
    return NULL;
}
...
nh_table_init(struct vrouter *router)
{
  ...
  if (!router->vr_max_nexthops) {
    ...
    router->vr_nexthops =
        vr_btable_alloc(router->vr_max_nexthops, sizeof(struct vr_nexthop *));
    if (!router->vr_nexthops)
 ...
 */
int
vr_nexthop_update_offload_vrfstats(uint32_t vrfid,
                                   uint32_t num_cntrs,
                                   uint64_t *cntrs)
{
  uint64_t *dst_cntr;
